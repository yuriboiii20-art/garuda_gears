...
document.querySelector('#app').innerHTML = `${header(currentPath)}<main id="main-content" tabindex="-1">${page ? (page.path === '/' ? homePage() : placeholderPage(page)) : notFoundPage()}</main>${page && (page.path === '/' || page.path === '/contact/') ? locationMap() : ''}${footer(currentPath)}`;
